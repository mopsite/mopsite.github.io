---
title: 'ES6 中 Class 的基本语法'
date: '2024-6-10'
category: '前端'
tags: ['javascript']
detail: 'ES6 引入了 Class（类）这个概念，作为对象的模板。通过 class 关键字可以定义类。'
---
...
上面代码中，`p1` 和 `p2` 都是 `Point` 的实例，它们的原型都是 `Point.prototype`，所以 `__proto__` 属性是相等的。这也意味着，可以通过实例的 `__proto__` 属性为“类”添加方法。
...
[ES2022](https://github.com/tc39/proposal-class-fields) 为类的实例属性又规定了一种新写法。实例属性现在除了可以定义在 `constructor()` 方法里面的 `this` 上，也可以定义在类内部的最顶层。
...
目前，只有这种写法可行，因为 ES6 明确规定，Class 内部只有静态方法，没有静态属性。现在有一个[提案](https://github.com/tc39/proposal-class-fields)提供了类的静态属性，写法是在实例属性的前面加上 `static` 关键字。
...
::: warning 提示
从 [Chrome 111](https://developer.chrome.com/blog/new-in-devtools-111/#misc) 开始，开发者工具里面可以读写私有属性，不会报错，原因是 Chrome 团队认为这样方便调试。
:::
...
上面代码中，`#x` 就是私有属性，在 `Point` 类之外是读取不到这个属性的。由于 `#` 是属性名的一部分，使用时必须一起，所以 `#x` 和 `x` 是两个不同的属性。
...
上面示例中，静态属性 `y` 和 `z` 的值依赖于静态属性 `x` 的运算结果，这段初始化逻辑写在类的外部（`try...catch` 结构中）。另一种方法是写到类的 `constructor()` 方法里面。这两种方法都不是很理想，前者是将类的内部逻辑写到了外部，后者则是每次新建实例都会运行一次。
...
上面代码中，静态块将静态属性 `y` 和 `z` 的初始化逻辑，写入了类的内部，而且只运行一次。
