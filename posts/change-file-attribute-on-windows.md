...
category: '系统'
tags: ['windows']
detail: '使用本指南中的方法，可以调整 Windows 文件的日期和其他元数据。'
---
...
首先，在 Windows 搜索中输入“PowerShell”，右键单击，并**以管理员身份运行**。
...
如果 PowerShell 没有显式任何错误，并再次将你带到同一目录，则确认属性已成功更改。
...
2. 下载属性更改器的完整安装程序（Full setup），不要下载便携式版本（Portable），因为它可能无法正常运行。
3. 下载软件后，运行安装文件并按照屏幕上的说明进行安装。
4. 如果软件要求，重新启动你的设备，否则没有必要。
5. 导航到包含要修改其属性的文件的文件夹。
6. 右键单击该文件，然后从上下文菜单中选择【更改属性】以打开该软件。如果你使用的是 Windows 11，可能需要点击【显式更多选项】才能在菜单中显示此选项。
...
6. 要删除选定的属性，请选中【从此文件中删除以下属性】旁边的圆圈，选择要删除的属性，然后单击【确定】。
