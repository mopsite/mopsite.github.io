...
2. 对象
   ...
   上面代码的 `valueOf()` 方法和 `toString()` 方法，返回的都是对象，所以转成数值时会报错。
...
2. 对象
   ...
   上面代码对三个对象使用 `String()` 函数。第一个对象返回 `toString()` 方法的值，第二个对象返回的还是 `toString()` 方法的值，第三个对象表示 `toString()` 方法优先于 `valueOf()` 方法。
...
下面这个例子，条件部分的每个值都相当于 `false`，使用否定运算符后就变成了 `true`：
