...
上面代码中，错误堆栈的最内从是 `throwit` 函数，然后是 `catchit` 函数，最后是函数的运行环境。
...
上面代码中，`try` 代码块抛出错误，JavaScript 引擎就立即把代码的执行转到 `catch` 代码块，或者说错误被 `catch` 代码块捕获了。`catch` 接受一个参数，表示 `try` 代码块抛出的值。
